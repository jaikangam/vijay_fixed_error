...
<div class="gmapfp item-page<?php echo $this->params->get('pageclass_sfx')?>">
	<div class="pull-right">
		<?php
			...
			if (($row->email!=null)and($this->params->get('gmapfp_msg')==1)) {
				...
				echo '<a href="'.$link.'" class="lightboxgmafp" rev="width:'.$width_msg.' height:'.$height_msg.' scrolling:no controlsPos:br" title="'.JText::_('GMAPFP_DISPLAY_MSG').'"><img src='.$msg.' /></a>&nbsp;&nbsp;&nbsp;';
			};
	...
	<div class="gmapfp_detail">
		<div class="gmapfp_taille1 pull-left">
			<?php
				if ($row->img!=null) { ?> <a class="lightboxgmafp" href='<?php echo $image; ?>' ><img src=<?php echo $image; ?> style="height:<?php echo $this->params->get('gmapfp_hauteur_img')?>px;"/><img src=<?php echo $loupe; ?> style="height:18px;"/></a> <?php }; ?>
		</div>
	...
	<?php if ($layout!='article') {?>
		<div class="gmapfp_article">
			...
			if ($row->message!=null) {
				...
				echo '<p class="readmore"><a class="lightboxgmafp" rev="width:650 height:500 disableScroll:true controlsPos:br" href="'.$link.'">'.JText::_('GMAPFP_LIRE_SUITE').'</a></p>';
			}; ?>
		</div>
	<?php } ?>
</div>